    ...
    return (
      <div className="card bg-base-100 shadow-xl border border-gray-300"
          ...
          <div className="card-body">
              <h2 className="card-title text-base">{auction.title}</h2>
          </div>
          ...
          <div className="card-body">
              <p className="text-sm text-[#62CBC6]">시작 시간: {getKrDateFormat(auction.startedAt)}</p>
              <p className="text-sm text-[#62CBC6]">종료 시간: {getKrDateFormat(auction.endedAt)}</p>
              <p className="text-sm text-[#62CBC6]">현재가: {getPriceFormatted(auction.price)}</p>
          </div>
      </div>
  );
}
